                    ...
                    <div className='category-diplay'>
                        {datas.basedOnCategory?.map((item, index) => (
                            <div key={index} className="box">
                                ...
                                <p style={{ marginLeft: '10px', color: 'black' }}>₹ {item.idMeal - randomSubtraction}<span className='addtocartp' style={{ marginLeft: '70px', color: 'black', cursor: 'pointer', backgroundColor: '#FFA500', padding: '5px' }} onClick={() => addCart(item.idMeal - randomSubtraction, item.strMealThumb, item.strMeal)}>Add to cart</span></p>
                            </div>
                        ))}
                    </div>
                </div> : <div></div>
